...
const responsibleUserId = computed<number | undefined>({
  get: () => form.responsibleUserIds[0],
  set: (value) => {
    form.responsibleUserIds = value == null ? [] : [value]
  },
})
...
function fillFormByDetail(detail: BugReportDetailOutput): void {
  ...
  // 历史简报可能保存过多个责任人；编辑时收敛为一个，后续保存即完成数据修正。
  form.responsibleUserIds = detail.responsibleUsers?.[0]?.userId
    ? [detail.responsibleUsers[0].userId]
    : []
...
async function autoFillFromTickets(ticketIds: number[]): Promise<void> {
  ...
  try {
    ...
    if (firstTicket) {
      ...
      // 默认使用最后一个关联工单的当前处理人；用户手动选择后不再追加或覆盖。
      const lastAssigneeId = details[details.length - 1]?.assigneeId
      if (form.responsibleUserIds.length === 0 && lastAssigneeId) {
        form.responsibleUserIds = [lastAssigneeId]
      }
    }
  } finally {
