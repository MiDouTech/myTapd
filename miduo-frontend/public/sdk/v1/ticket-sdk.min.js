  ...
  class TicketSdkImpl {
    ...
    openModal(myTickets = false, prefillDescription, autoCaptured = false) {
      ...
      if (myTickets) {
        ...
        updateSubmitState();
        (_d = panel.querySelector('[data-action="pick-attachment"]')) == null ? void 0 : _d.addEventListener("click", () => {
        ...
        (_e = panel.querySelector('[data-action="submit"]')) == null ? void 0 : _e.addEventListener("click", () => {
          void this.submitTicket(panel, uploadedAttachments);
        });
        (_f = panel.querySelector('[data-action="open-my-tickets"]')) == null ? void 0 : _f.addEventListener("click", () => {
          if (hasUnsavedDraft()) {
    ...
    renderAttachmentList(panel, attachments) {
      ...
      listEl.innerHTML = attachments.map((item, index) => `<div style="margin-bottom:4px;">${item.type === "video" ? "视频" : "图片"}${index + 1}：<a href="${escapeHtml(item.url)}" target="_blank" rel="noopener noreferrer">查看</a></div>`).join("");
    }
