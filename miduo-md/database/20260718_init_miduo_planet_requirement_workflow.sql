...
--
-- 误操作回退说明：
--   - 各状态均配置 isReturn=true 的退回流转，工单详情页以黄色按钮展示
--   - 若当前状态存在多条退回路径（如「处理中」），请点选对应来源状态的退回按钮
--   - 已部署旧版（28 流转）请执行 20260718_patch_miduo_planet_requirement_workflow_undo.sql
--
...
INSERT INTO `workflow` (
    ...
    '米多星球需求工作流',
    'ADVANCED',
    '米多星球需求全生命周期流转：待评审→待规划→待调研→方案中→开发中→已完成；支持技术问题直转处理中、无需处理/无效终态、重新激活及各环节误操作回退',
    '[
