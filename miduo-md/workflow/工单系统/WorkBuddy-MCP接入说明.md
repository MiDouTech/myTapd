...
## 2. 第一步：创建密钥并一键复制 MCP 配置（推荐）

1. 浏览器登录 **工单 Web**（与日常办公相同账号）。
2. 右上角用户菜单 → **个人 API 密钥**（路由：`/account/api-keys`）。
3. **新建密钥** → 在弹窗中可直接看到两块内容：
   - **完整密钥**（仅显示一次，形如 `mdt_xxxx_yyyy`）；
   - **WorkBuddy / MCP 一键配置**：已自动内联本次密钥与正确的 `/api/mcp` 地址。
4. 点击 **「一键复制 MCP 配置」**，得到可直接粘贴的完整 JSON，**无需自己拼接地址与密钥**。
5. **不要**把密钥/配置贴进群聊、评论、Wiki 或 Git。

> 弹窗关闭后无法再次查看完整密钥；如未保存，请重新创建一个密钥即可重新获得一键配置。

---

## 3. 第二步：在 WorkBuddy 粘贴 MCP 配置

WorkBuddy → **设置 → 连接器（MCP）→ 添加 MCP Server**，把上一步复制的内容直接粘贴。其形如：
...
- `url` 指向 `/api/mcp`（一键配置已按当前站点自动生成绝对地址）。
