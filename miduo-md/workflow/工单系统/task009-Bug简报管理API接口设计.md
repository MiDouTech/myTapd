...
2. 创建/更新支持多工单关联，并可按缺陷工单信息自动预填简报字段；其中缺陷等级优先取缺陷工单测试信息 `severityLevel`，来源包括「确认缺陷转开发」时保存的等级。  
...
6. **缺陷等级带入**：缺陷转开发时必填的 `severityLevel(P0-P4)` 会保存到测试信息；Bug 简报自动草稿和编辑页关联工单预填时都应自动带入该等级。  
