...
# 参数说明:
#   环境       - test / prod (必填)
#   命名空间ID - Nacos namespace ID (可选)
#   nacos地址  - (可选，默认 http://10.0.4.4:8848)
#   用户名     - (可选，默认 nacos)
#   密码       - (可选，默认 nacos)
#
# 示例:
#   bash nacos-import.sh test
#   bash nacos-import.sh prod
# ============================================================

set -euo pipefail

ENV="${1:?请指定环境: test / prod}"
...
DATA_ID="ticket-platform-${ENV}.yaml"
GROUP="DEFAULT_GROUP"
...
if [ -z "$ACCESS_TOKEN" ]; then
    echo "[INFO] 无法获取 accessToken，尝试无鉴权导入"
    AUTH_PARAM=""
else
    echo "[OK] 登录成功"
...
echo "============================================"
echo " 导入完成!"
echo " 启动时指定 --spring.profiles.active=${ENV} 即可"
echo "============================================"
