...
public class TicketApplicationService {
    ...
    private TicketPublicDetailOutput buildPublicDetailOutput(TicketPO ticket) {
        ...
        if (customerInfoOutput != null) {
            ...
            // 与前端工单详情「已选问题截图」一致：库表 problem_screenshot + 企微机器人关联的图片附件
            // （企微先文后图时图片只写入 ticket_attachment，字段可能为空）
     ...
     * 库表 {@code problem_screenshot} 中的 URL，加上企微机器人写入且未体现在字段里的图片附件（先文后图建单场景）。
     */
