 ...
 * 无效反馈月报管理控制器（并入日报管理）
 */
@Tag(name = "无效反馈月报管理", description = "无效反馈月报预览、推送、兼容配置管理")
@RestController
...
public class WeeklyInvalidReportController {
    ...
    /**
     * 预览无效反馈月报（不推送，仅返回数据和Markdown）
     * 接口编号：API000517
     * 产品文档功能：无效反馈月报 - 预览月报内容（并入日报管理页）
     */
    @Operation(summary = "预览无效反馈月报", description = "接口编号：API000517")
    @GetMapping("/preview")
     ...
     */
    @Operation(summary = "手动推送无效反馈月报", description = "接口编号：API000518")
    @PostMapping("/push")
     ...
     */
    @Operation(summary = "查询无效反馈报表配置（兼容）", description = "接口编号：API000519")
    @GetMapping("/config")
     ...
     */
    @Operation(summary = "更新无效反馈报表配置（兼容）", description = "接口编号：API000520")
    @PutMapping("/config")
